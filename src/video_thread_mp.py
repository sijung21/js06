...
def producer(q):
    ...
    rear_cap = cv2.VideoCapture('rtsp://admin:sijung5520@192.168.100.100/profile2/media.smp')
    front_cap = cv2.VideoCapture('rtsp://admin:sijung5520@192.168.100.101/profile2/media.smp')
    ...
    if rear_cap.isOpened() and front_cap.isOpened():
        while True:
            ...
            if epoch[-1:] == '0':
                ...
                front_ret, front_frame = rear_cap.read()
                rear_ret, rear_frame = front_cap.read()

                if not front_ret and not rear_ret:
                    rear_cap.release()
                    front_cap.release()
                    rear_cap = cv2.VideoCapture('rtsp://admin:sijung5520@192.168.100.100/profile2/media.smp')
                    front_cap = cv2.VideoCapture('rtsp://admin:sijung5520@192.168.100.101/profile2/media.smp')
                ...
                visibility_rear = target_info.minprint(epoch[:-2], front_left_range, front_right_range,
                                                       front_distance, front_frame, rear_cap_name)
                ...
                cv2.imwrite(f'{image_save_path}/resize/{front_cap_name}/{date}/{epoch}.jpg', front_frame)
                cv2.imwrite(f'{image_save_path}/resize/{rear_cap_name}/{date}/{epoch}.jpg', rear_frame)
